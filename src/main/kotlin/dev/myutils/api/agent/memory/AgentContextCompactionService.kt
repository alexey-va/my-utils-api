...
class AgentContextCompactionService(
	...
	private companion object {
		val COMPACT_SYSTEM_PROMPT =
			...
			Сохрани: темы, решения, числа (веса, абсолютные даты), просьбы пользователя.
			Не используй без абсолютной даты относительные формулировки «сегодня», «вчера», «эта неделя».
			Не сохраняй как актуальный факт переходящие выводы «уже сделали», «осталось», «группа закрыта»:
			текущее состояние недели всегда приходит агенту отдельно в свежем снимке дневника.
