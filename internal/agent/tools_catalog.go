...
func ToolDefinitions(temporalEnabled bool) []openrouter.Tool {
	...
	tools := []openrouter.Tool{
		...
		definition("copy_workout", "Скопировать подходы из дневника. Без source_date — последняя сессия ДО date. При исправлении веса source_date=date сохраняет подходы этой записи. Для переноса даты копируй с явной source_date, затем удали исходную запись в том же плане. Для относительного веса используй weight_delta_kg; для новых повторений с прошлым весом — repetitions. Итоговый вес вычисляет сервер. Не сочиняй notation из summary.", property{"exercise_name": text("Упражнение"), "date": text("Целевая дата YYYY-MM-DD"), "source_date": text("Точная дата источника YYYY-MM-DD, только для выбранной пользователем записи"), "weight_kg": property{"type": "number", "description": "Новый абсолютный вес в кг, только если пользователь его назвал; несовместим с weight_delta_kg"}, "weight_delta_kg": property{"type": "number", "description": "Изменение прошлого веса в кг: +4 для на 4 кг больше, -2 для на 2 кг меньше. Не итоговый вес. Несовместим с weight_kg"}, "repetitions": text("Новые повторения без веса: по умолчанию пользовательское A/B передавай как 3*A/B, включая A/A. Примеры 3*10/10, 3*10/12, 8/8/8. Явные два сета — 10/10. Без поля сохраняются подходы источника")}, "exercise_name", "date"),
