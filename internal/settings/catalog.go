...
func AppCatalog(reminderChanged ApplyFunc) Catalog {
	...
	return NewCatalog([]Definition{
		...
		Int(AgentRecentMessages, "Сколько последних пользовательских ходов целиком подставлять в LLM, включая все вызовы и результаты. Полная история хранится в Postgres.", []string{"agent"}, 10, 1, 50, nil),
		...
		Int(AgentCompactThreshold, "После скольких завершённых пользовательских ходов сверх recent tail запускать авто-сжатие.", []string{"agent"}, 40, 5, 500, nil),
